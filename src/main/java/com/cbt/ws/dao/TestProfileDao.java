...
public class TestProfileDao {
	...
	public TestProfile[] getAll() {
		List<TestProfile> TESTPROFILEurations = new ArrayList<TestProfile>();
		...
		for (Record r : result) {
			...
			TESTPROFILEurations.add(tc);
			mLogger.debug(tc);
		}
		return TESTPROFILEurations.toArray(new TestProfile[TESTPROFILEurations.size()]);
	}
	...
	public Long add(TestProfile testProfile) {
		...
		Long TESTPROFILEID = sqexec.insertInto(TESTPROFILE, TESTPROFILE.MODE, TESTPROFILE.DEVICETYPECOUNT)
				.values(testProfile.getMode(), testProfile.getDeviceTypes().size())
		...
		for (Integer deviceTypeId : testProfile.getDeviceTypes()) { 
			mLogger.trace("Addign device id");
			//TODO: improve performance here
			sqexec.insertInto(TESTPROFILE_DEVICES, TESTPROFILE_DEVICES.TESTPROFILE_ID, TESTPROFILE_DEVICES.DEVICE_TYPE_ID)
					.values(TESTPROFILEID, deviceTypeId).execute();
		}
